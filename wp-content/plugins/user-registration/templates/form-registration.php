...
<br>
	...
	<div class="col-7 signupbg">
		<div class="container">
			...
			<div class="row">
				...
				<h3 class="earthlyrubik" style="font-weight:400;">Register</h3>
				<p class="earthlyrubik">It's nice to meet you! We're Earthly Vibes. Care to introduce yourself? </p>
				</div>
				<div class="col-md-4">
				</div>
			</div>
	...
	<div class='col-5 user-registration ur-frontend-form <?php echo esc_attr( $template_class ) . ' ' . esc_attr( $custom_class ); ?>' id='user-registration-form-<?php echo absint( $form_id ); ?>' style="border:0px solid;">
		<form method='post' class='register' data-form-id="<?php echo absint( $form_id ); ?>"
										...
										if ( isset( $single_item->field_key ) ) {
											...
											?>
												<div <?php echo $cl_props; ?> data-field-id="<?php echo esc_attr( $field_id ); ?>" class="ur-field-item field-<?php echo esc_attr( $single_item->field_key ); ?> <?php echo esc_attr( ! empty( $single_item->advance_setting->custom_class ) ? $single_item->advance_setting->custom_class : '' ); ?>">
													...
													?>
													<div class="form-row validate-required" id="<?php echo $single_item->field_key->general_setting->field_name.'_field';?>" data-priority="">
												...
												<?php if($grid_data_key	==	1){?>
													<div <?php echo $cl_props; ?> data-field-id="phone" class="ur-field-item field-phone <?php echo esc_attr( ! empty( $single_item->advance_setting->custom_class ) ? $single_item->advance_setting->custom_class : '' ); ?>">
													<div class="form-row validate-required" id="phone_field">
														<span class="input-wrapper"> 
															<input data-rules="" data-id="mobile" type="tel" class="form-control input-text without_icon custominput input-text ur-frontend-field " name="mobile" id="mobile" placeholder="+91-00000 00000" value="" required="required" pattern="[0-9]{3}[0-9]{5}[0-9]{5}"><!--pattern="[0-9]{3}-[0-9]{5} [0-9]{5}"--> 
														</span> 
													</div>
												</div>
												<?php }?>
			...
			if ( $is_field_exists ) {
				?>
					...
					?>
					<div class="ur-button-container <?php echo esc_attr( implode( ' ', $btn_container_class ) ); ?>" style="padding: 0px 0px 0px 10px;">
						<?php
						...
						?>
						<button type="submit" class="signupBtn btn button ur-submit-button <?php echo esc_attr( implode( ' ', $submit_btn_class ) ); ?>">
							<span></span>
							<?php
							$submit = ur_get_form_setting_by_key( $form_id, 'user_registration_form_setting_form_submit_label' );
								echo esc_html( ur_string_translation( $form_id, 'user_registration_form_setting_form_submit_label','Continue' ) );
							?>
						</button>
					...
					<div style="padding:15px;">
						<p style="color:#13473C">Already have on account? <a href="<?php echo esc_url( '/earthly/my-account' ); ?>" style="color:#13473C"><u>Login</u></a></p>
					</div>
					<?php
			}
...
<script>
	jQuery(document).ready(function(){
		jQuery("div").removeClass("entry-content");
		jQuery("div").removeClass("content-area");
		jQuery("#content-inside").removeClass("container");
		jQuery("#content-inside").addClass("container-fluid");
	});
</script>
<?php
