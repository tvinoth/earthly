 ...
 */
function woolentor_render_icon( $settings = [], $new_icon = 'selected_icon', $old_icon = 'icon', $attributes = [],$flagtypeicon = "" ){
    ...
    if ( woolentor_is_elementor_version( '>=', '2.6.0' ) && ( $is_new || $migrated ) ) {
        ...
        if ( 'svg' === $settings[$new_icon]['library'] ) {
			...
			if($flagtypeicon	==	"open"){
				$output = '<img src="' . $settings[$new_icon]['value']['url'] . ' " height="12" width="12"></img>';
			}else{
				$output = '<img src="' . $settings[$new_icon]['value']['url'] . '" height="15" width="16"></img>';	
			}
